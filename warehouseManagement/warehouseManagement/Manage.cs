...
namespace warehouseManagement
{
    public partial class Manage : Form
    {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            string insertQuery = "INSERT INTO record3 (FIRSTNAME, LASTNAME, DOB, CONTACTNUM, USERNAME, PASSWORD) VALUES (@FIRSTNAME, @LASTNAME, @DOB, @CONTACTNUM, @USERNAME, @PASSWORD)";
            string selectQuery = "SELECT * FROM record3";
            ...
            using (MySqlConnection con = new MySqlConnection(connectionString))
            {
                using (MySqlCommand cmd = new MySqlCommand(insertQuery, con))
                {
                    ...
                    cmd.Parameters.AddWithValue("@CONTACTNUM", this.textBox3.Text);
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                string connectionString = "server=localhost;user id=root;password=;database=register";
                string deleteQuery = "DELETE FROM record3 WHERE ID = @ID";
                string selectQuery = "SELECT * FROM record3";
                ...
                try
                {
                    using (MySqlConnection con = new MySqlConnection(connectionString))
                    {
                        using (MySqlCommand cmd = new MySqlCommand(deleteQuery, con))
                        {
                            ...
                            con.Open();
                            cmd.ExecuteNonQuery();
                            MessageBox.Show("User has been deleted.", "Notice");
            ...
            {
                MessageBox.Show("Please select a row to delete.", "Update");
            }
        }
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                string connectionString = "server=localhost;user id=root;password=;database=register";
                string updateQuery = "UPDATE record3 SET FIRSTNAME = @FIRSTNAME, LASTNAME = @LASTNAME, DOB = @DOB, CONTACTNUM = @CONTACTNUM, USERNAME = @USERNAME, PASSWORD = @PASSWORD WHERE ID = @ID";
                string selectQuery = "SELECT * FROM record3";
        ...
        {
            string connectionString = "server=localhost;user id=root;password=;database=register";
            string query = "UPDATE INTO record3 (FIRSTNAME, LASTNAME, DOB, CONTACTNUM, USERNAME, PASSWORD) VALUES (@FIRSTNAME, @LASTNAME, @DOB, @CONTACTNUM,@USERNAME, @PASSWORD)";
            string selectQuery = "SELECT * FROM record3";
            using (MySqlConnection con = new MySqlConnection(connectionString))
            {
                using (MySqlCommand selectCmd = new MySqlCommand(selectQuery, con))
